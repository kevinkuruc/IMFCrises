...
CurrN = N[2]
DebtN = N[3]
bar(["Banking \n ($BankN)", "Currency \n ($CurrN)", "Debt \n ($DebtN)"], Averages, grid=false, color=treatedblue, label="", fontfamily="Times", ylabel="Estimated Cumulative Effect")
...
MidEastN = N[6]
#NorthAmN = N[7]
bar(["Africa \n ($AfricaN)", "South Asia \n ($SouthAsiaN)", "East Asia & \n Pacific \n ($EastAsiaN)", "Europe & \n Central Asia \n ($EuropeN)", "Latin Am. \n ($LatAmN)", "Mid. East \n ($MidEastN)"], Averages, color=treatedblue, label="", fontfamily="Times", grid=false, ylabel="Estimated Cumulative Output Effect \n (% of crisis year GDP)", guidefontsize = 8)
