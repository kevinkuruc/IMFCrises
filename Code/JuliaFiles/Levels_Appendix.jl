...
plot!(t[6:end], [0; Level_Betas[:, 1]], linewidth=1.9, color=:gray, style=:dash, label="Level Analysis (Comparison of Cumulative Growth)", marker=([:hexagon], [:gray], [2.5]))
plot!(t[6:end], [100*(Levels_Outcomes[6,1]/Levels_Outcomes[6,2] -1); Level_Betas[:, 2]], linewidth=1.9, color=:gray, style=:dot, label="Level Analysis (Comparison of Raw Levels)", marker=([:xcross], [:gray], [2.5]), guidefont="Times", legendfont="Times")
hline!([0], color=:black, style=:dot, label="")
savefig(joinpath(output_directory, "Levels_IRF_500K.pdf"))
savefig(joinpath(output_directory, "Levels_IRF_500K.svg"))
