...
Synthetics_SomeDrawn        = join(Synthetics, Treated_SomeDrawn, on=:TempID, kind=:inner, makeunique=true) 
...
for (h, pg) in enumerate(PostGrowths)
      ...
      TempDiff = convert(Array, Treated_SomeDrawn[pg]-Synthetics_SomeDrawn[pg])
...
plot(t, [0; MainBetas[:,1]], linewidth=2.5, color=treatedblue, grid=false, label="", ylabel="", xlabel="Years From Crisis", legend=(0.1,0.385), foreground_color_legend=nothing, background_color_legend=nothing, legendfontsize=7, ylims=(-3,4.0), marker=([:circle], [treatedblue], [2.8]))
...
plot(t, [0; MainBetas[:,1]], linewidth=2.5, color=treatedblue, label="", ylabel="", grid=false, xlabel="Years From Crisis", legend=(0.1,0.3), legendfontsize=7, foreground_color_legend=nothing, ylims=(-2,3.5), marker=([:circle], [treatedblue], [2.5]))
...
plot!(t, RobustnessChecks[2:end, 15], linewidth=1.9, color=g7, style=:dot, label="+/-20",  marker=([:pentagon], [g7], [2.5]))
hline!([0], color=:black, style=:dot, label="")
